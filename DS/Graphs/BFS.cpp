//Procedure for BFS is same for directed/undirected graphs.
...
class Graph{
    ...
    public:
        ...
        void BFS(int s)     //prints BFS traversal from a given source s. Understand it as: it pushes each level into the queue,and only then does it go deeper,into the next level(adjacent vertices of the current node). TOH ye bfs hi hua.
        {
            bool *visited=new bool[V];      //To avoid processing a node more than once, we use a boolean visited array. For simplicity, it is assumed that all vertices are reachable from the starting vertex. VERY IMP ASSUMPTION.
            for(int i=0;i<V;i++)
